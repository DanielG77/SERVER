...
public class CohereProvider implements AiProvider {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CohereProvider.class);
    private static final String NAME = "cohere";
    ...
    public String generateExplanation(String query, String tournamentData) throws Exception {
        ...
        String requestBody = String.format(
            "{\"prompt\":\"%s\",\"max_tokens\":30,\"temperature\":0.8}",
            escapeJson(prompt)
        );
    ...
    public SearchIntent interpretQuery(String naturalLanguageQuery) throws Exception {
        ...
        String requestBody = String.format(
            "{\"model\":\"command\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"max_tokens\":500}",
            escapeJson(prompt)
        );
    ...
    public boolean isHealthy() {
        ...
        try {
            String requestBody = "{\"prompt\":\"hi\",\"max_tokens\":5}";
            HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(API_URL))
            ...
            logger.warn("Cohere health check failed", e);
            return false;
        }
    }
    ...
    private String extractContent(String response) {
        ...
        return response.substring(start + 8, end);
    }
