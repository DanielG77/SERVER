...
public class OpenRouterProvider implements AiProvider {
    ...
    public String generateExplanation(String query, String tournamentData) throws Exception {
        ...
        String requestBody = String.format(
            "{\"model\":\"openrouter/auto\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"max_tokens\":30}",
            escapeJson(prompt)
        );
    ...
    public SearchIntent interpretQuery(String naturalLanguageQuery) throws Exception {
        ...
        String requestBody = String.format(
            "{\"model\":\"openrouter/auto\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"max_tokens\":500}",
            escapeJson(prompt)
        );
    ...
    public boolean isHealthy() {
        ...
        try {
            String requestBody = "{\"model\":\"openrouter/auto\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}],\"max_tokens\":5}";
            HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(API_URL))
            ...
            logger.warn("OpenRouter health check failed", e);
            return false;
        }
    }
