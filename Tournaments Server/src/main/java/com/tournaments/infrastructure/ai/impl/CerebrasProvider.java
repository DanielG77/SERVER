...
public class CerebrasProvider implements AiProvider {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CerebrasProvider.class);
    private static final String NAME = "cerebras";
    ...
    public String generateExplanation(String query, String tournamentData) throws Exception {
        if (apiKey == null || apiKey.isEmpty()) {
            throw new IllegalStateException("Cerebras API key not configured");
        }
        ...
        String requestBody = String.format(
            "{\"model\":\"llama-3.1-70b\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"max_tokens\":30}",
            escapeJson(prompt)
        );
        ...
        if (response.statusCode() != 200) {
            throw new RuntimeException("Cerebras API error: " + response.body());
        }
    ...
    public SearchIntent interpretQuery(String naturalLanguageQuery) throws Exception {
        ...
        String requestBody = String.format(
            "{\"model\":\"llama-3.1-70b\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"max_tokens\":500}",
            escapeJson(prompt)
        );
    ...
    public boolean isHealthy() {
        ...
        try {
            String requestBody = "{\"model\":\"llama-3.1-70b\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}],\"max_tokens\":5}";
            HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(API_URL))
            ...
            logger.warn("Cerebras health check failed", e);
            return false;
        }
    }
