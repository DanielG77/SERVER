...
public class GroqProvider implements AiProvider {
    ...
    public String generateExplanation(String query, String tournamentData) throws Exception {
        ...
        String prompt = String.format(
            ...
            "Búsqueda: %s\n\n" +
            "Torneo: %s\n\n" +
            "Responde solo con la explicación, sin prefijos.",
            query, tournamentData
        );

        String requestBody = String.format(
            "{\"model\":\"mixtral-8x7b-32768\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"max_tokens\":30}",
            escapeJson(prompt)
        );
    ...
    public boolean isHealthy() {
        ...
        try {
            String requestBody = "{\"model\":\"mixtral-8x7b-32768\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}],\"max_tokens\":5}";
            HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(API_URL))
            ...
            logger.warn("Groq health check failed", e);
            return false;
        }
    }
